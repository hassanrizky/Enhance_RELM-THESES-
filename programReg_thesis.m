...
tic;
modelReg = fitrlinear(X_train, Y_train);
trainingTime = toc;

tic;
Y_ValTest = predict(modelReg, X_val);
validationTime = toc;
...
tic;
Y_OutTest = predict(modelReg, X_test);
executionTime = toc;
...
disp(['RMSE Val RL: ', num2str(rmseVal)]);
disp(['MAE Val RL: ', num2str(maeVal)]);
disp(['MAPE Val RL: ', num2str(mapeVal)]);
disp(['R-Square Val RL: ', num2str(rsquareVal)]);
disp(' ');
disp('Testing Data');
disp(['RMSE Test RL: ', num2str(rmseTest)]);
disp(['MAE Test RL: ', num2str(maeTest)]);
disp(['MAPE Test RL: ', num2str(mapeTest)]);
disp(['R-Square Test RL: ', num2str(rsquareTest)]);
disp(' ');
disp('Execution Time');
disp(['Training Time RL: ', num2str(trainingTime)]);
disp(['Validation Time RL: ', num2str(validationTime)]);
disp(['Testing Time RL: ', num2str(executionTime)]);
disp(' ');
...
for fold = 1:k
    ...
    modelGAM = fitrlinear(X_train, Y_train);
...
%%
% Visualization (for example, bar plots)
figure;
bar([Y_test, Y_OutTest]);
title('Average Performance Metrics');
xlabel('Metrics');
ylabel('Average Value');
legend({'Test Aktual', 'Test Prediction'});